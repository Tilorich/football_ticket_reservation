...
//Empty Fields and empty data   
if((empty($order)) || (empty($destination)) ||(empty($travelclass)) ||(empty($seats)) ||(empty($fullname)) ||(empty($amount)) ||(empty($acct)) ||(empty($paymethod))){    
...
//ending
if(empty($message)){
$insertdata=mysqli_query($conn,"INSERT into booking_details (order_ref,fullname,contact,gender,class_reserved,destination,seats_reserved,date_reserved,transaction_id,account,amount) VALUES('$order','$fullname','$contact','$gender','$travelclass','$destination','$seats','$traveldate','$acct','$paymethod','$amount')");    
